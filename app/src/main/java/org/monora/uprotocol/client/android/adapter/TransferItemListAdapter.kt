...
class TransferItemListAdapter(
    fragment: IListingFragment<GenericItem, ViewHolder>,
) : ListingAdapter<GenericItem, ViewHolder>(fragment) {
    ...
    override fun onLoad(): MutableList<GenericItem> = ArrayList()

    class GenericItem : ContentModel {
    ...
    protected override fun onLoad(lister: GroupLister<GenericItem>) {
                    ...
                    if (!loadThumbnails) item.supportThumbnail = false else {
                        ...
                        if (format.size > 0 && ("image" == format[0] || "video" == format[0])) {
                            ...
                            if (isOutgoing) documentFile = Files.fromUri(
                                ...
                                org.monora.uprotocol.client.android.util.Files.getIncomingPseudoFile(
        ...
        if (currentPath == null && hasIncoming) try {
            ...
            val savePath = org.monora.uprotocol.client.android.util.Files.getSavePath(context, transfer)
    ...
    abstract class GenericItem : TransferItem {
    ...
    class GenericTransferItem : GenericItem {
        ...
        override fun getFirstText(adapter: TransferItemListAdapter): String {
            return Files.formatLength(getComparableSize(), false)
        }
    ...
    open class TransferFolder internal constructor(
        ...
        override fun getFirstText(adapter: TransferItemListAdapter): String? {
            return Files.formatLength(bytesTotal, false)
        }
    ...
    class StorageStatusItem : GenericItem(), StatusItem {
        ...
        override fun getFirstText(adapter: TransferItemListAdapter): String {
            return if (bytesFree == -1L) adapter.context
                .getString(R.string.text_unknown) else Files.formatLength(bytesFree, false)
        }
