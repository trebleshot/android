		...
		new WorkerService.RunningTask()
		{
			...
			public void onRun()
			{
				try {
					...
					final DocumentFile receivedFile = UpdateUtils.receiveUpdate(activity, device,
							getInterrupter(), socket -> CoolSocket.connect(client -> {
								try {
									CoolSocket.ActiveConnection activeConnection = client.connect(
											new InetSocketAddress(connection.ipAddress,
													AppConfig.SERVER_PORT_COMMUNICATION),
											AppConfig.DEFAULT_SOCKET_TIMEOUT);
									activeConnection.reply(new JSONObject()
											.put(Keyword.REQUEST, Keyword.BACK_COMP_REQUEST_SEND_UPDATE)
											.toString());
							...
							if (receivedFile == null)
								new AlertDialog.Builder(activity)
										...
										.setPositiveButton(R.string.butn_retry,
                                                (dialog, which) -> runReceiveTask(activity, device, connection))
										.show();
							else
								new AlertDialog.Builder(activity)
										...
										.setNegativeButton(R.string.butn_close, null)
										.setPositiveButton(R.string.butn_open,
                                                (dialog, which) -> FileUtils.openUriForeground(activity, receivedFile))
										.show();
						}
					});
				} catch (Exception e) {
