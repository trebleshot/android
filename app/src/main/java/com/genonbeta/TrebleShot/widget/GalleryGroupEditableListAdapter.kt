...
abstract class GalleryGroupEditableListAdapter<T : GalleryGroupShareable?, V : GroupViewHolder?>(
    ...
    groupBy: Int
) : GroupEditableListAdapter<T, V>(fragment, groupBy), CustomGroupLister<T> {
    override fun onCustomGroupListing(lister: GroupLister<T>, mode: Int, `object`: T): Boolean {
        if (mode == MODE_GROUP_BY_ALBUM) {
            lister.offer(`object`, StringMerger<T>(`object`.albumName))
            return true
        }
        return false
    }
    ...
    companion object {
        val MODE_GROUP_BY_ALBUM: Int = GroupEditableListAdapter.Companion.MODE_GROUP_BY_DATE + 1
    }
}