...
class ActiveConnectionListFragment :
    ...
    private val mReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (NetworkManagerFragment.Companion.WIFI_AP_STATE_CHANGED == intent.action || ConnectivityManager.CONNECTIVITY_ACTION == intent.action || WifiManager.WIFI_STATE_CHANGED_ACTION == intent.action || WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION == intent.action || BluetoothA2dp.ACTION_CONNECTION_STATE_CHANGED == intent.action) refreshList()
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mFilter.addAction(NetworkManagerFragment.Companion.WIFI_AP_STATE_CHANGED)
    ...
    override fun performDefaultLayoutClick(
        ...
        `object`: EditableNetworkInterface
    ): Boolean {
        WebShareDetailsDialog(
            requireActivity(), TextUtils.makeWebShareLink(
                requireContext(),
                Networks.getFirstInet4Address(`object`).getHostAddress()
            )
        ).show()
        return true
    }
    ...
    override fun performLayoutClickOpen(
        ...
        if (!super.performLayoutClickOpen(holder, `object`)) startActivity(
