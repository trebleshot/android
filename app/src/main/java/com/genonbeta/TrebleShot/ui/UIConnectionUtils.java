...
public class UIConnectionUtils
{
					...
					new Handler(Looper.getMainLooper()).post(() -> {
						if (!activity.isFinishing())
							new AlertDialog.Builder(activity)
									...
									.setMessage(activity.getString(R.string.text_notAllowedHelp,
											device.nickname, AppUtils.getLocalDeviceName(activity)))
								 ...
								 final NetworkDeviceLoader.OnDeviceRegisteredListener registerListener)
	{
		WorkerService.RunningTask runningTask = new WorkerService.RunningTask()
		{
			...
			{
				final DialogInterface.OnClickListener retryButtonListener =
						(dialog, which) -> makeAcquaintance(activity, task, object, accessPin, registerListener);

				try {
					if (object instanceof NetworkDeviceListAdapter.HotspotNetwork) {
						mRemoteAddress = getConnectionUtils().establishHotspotConnection(
								getInterrupter(), (NetworkDeviceListAdapter.HotspotNetwork) object,
								(delimiter, timePassed) -> timePassed >= 20000);
					} else if (object instanceof String)
						mRemoteAddress = (String) object;

					if (mRemoteAddress != null) {
						mConnected = setupConnection(activity, mRemoteAddress, accessPin,
								(database, device, connection) -> {
	...
	public void showConnectionOptions(final Activity activity, final int locationPermRequestId,
									  final RequestWatcher watcher)
	{
		if (!getConnectionUtils().getWifiManager().isWifiEnabled())
			...
			getSnackbarSupport().createSnackbar(R.string.mesg_scanningSelfHotspot)
					.setAction(R.string.butn_wifiSettings, view -> activity.startActivity(
							new Intent(Settings.ACTION_WIFI_SETTINGS).addFlags(
