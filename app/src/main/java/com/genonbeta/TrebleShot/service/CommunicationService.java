...
public class CommunicationService extends Service
{
    ...
    public static final String ACTION_START_TRANSFER = "com.genonbeta.intent.action.SEAMLESS_START";
    public static final String ACTION_STOP_TRANSFER = "com.genonbeta.TrebleShot.transaction.action.CANCEL_JOB";
    public static final String ACTION_TOGGLE_FAST_MODE = "com.genonbeta.TrebleShot.transaction.action.TOGGLE_SEAMLESS_MODE";
    ...
    public void onDestroy()
    {
        ...
        synchronized (getActiveProcessList()) {
            for (ProcessHolder processHolder : getActiveProcessList()) {
                processHolder.interrupter.interrupt();
                Log.d(TAG, "onDestroy(): Killing sending process: " + processHolder.toString());
            }
        }
    }
    ...
    {
        Log.d(TAG, "Beginning of process: handleTransferAsSender");
        ...
        boolean sessionSuccessful = true;
        boolean retry = false;
        ...
        try {
            while (processHolder.activeConnection.getSocket().isConnected()) {
                ...
                try {
                    TransferObject firstAvailableTransfer = TransferUtils.fetchFirstValidIncomingTransfer(
                    ...
                    processHolder.object = firstAvailableTransfer;
...

                    {
...

                        {
                            ...
                            Log.d(TAG, "Receive.onTaskPrepareSocket(): reply: " + reply.toString());
                        }
                    ...
                    if (processHolder.object != null) {
                        Log.d(TAG, "SeamlessClientHandler.onConnect(): Updating file instances to "
            ...
            try {
                ...
                boolean hasLeftFiles = getDatabase().getFirstFromTable(TransferUtils.createIncomingSelection(
                ...
                processHolder.activeConnection.reply(new JSONObject()
                        ...
                        .put(Keyword.TRANSFER_JOB_DONE, sessionSuccessful && hasLeftFiles)
                        .toString());
                Log.d(TAG, "SeamlessClientHandler.onConnect(): reply: done ?? " + (sessionSuccessful && hasLeftFiles));
    ...
    public void handleTransferAsSender(ProcessHolder processHolder)
    {
        ...
        try {
            ...
            while (processHolder.activeConnection.getSocket().isConnected()) {
                {
                    ...
                    try {
                        if (request.has(Keyword.RESULT) && !request.getBoolean(Keyword.RESULT)) {
                            if (request.has(Keyword.TRANSFER_JOB_DONE) && request.getBoolean(Keyword.TRANSFER_JOB_DONE))
                                Log.d(TAG, "SeamlessServer.onConnected(): Receiver notified us " +
                                        "that it has received all the pending transfers: " + processHolder.device.id);
                            else
                                processHolder.interrupter.interrupt();

                            return;
                        } else if (!processHolder.interrupter.interrupted()) {
                            processHolder.object = new TransferObject(processHolder.group.id, request.getInt(Keyword.TRANSFER_REQUEST_ID),
                                    processHolder.type);
                        ...
                        Log.d(TAG, "SeamlessServer.onConnected(): File is not accessible ? " + processHolder.object.name);
                    ...
                    if (reply.has(Keyword.RESULT) && reply.getBoolean(Keyword.RESULT) && inputStream != null) {
                        int readLength;
                ...
                if (processHolder.object != null) {
                    Log.d(TAG, "SeamlessServer.onConnected(): Updating file instances to "
                            + processHolder.object.getFlag(processHolder.device.id));
                    getDatabase().update(processHolder.object);
                }
            }
        } catch (Exception e) {
            ...
            if (!sessionSuccessful && !processHolder.interrupter.interrupted())
            ...
            DeviceNotFoundException, ConnectionNotFoundException, AssigneeNotFoundException
    {
        ...
        Log.d(TAG, "Starting as client with deviceId=" + processHolder.device.id + " groupId="
        ...
        protected void onConnected(final ActiveConnection activeConnection)
        {
            ...
            try {
                ...
                if (deviceSerial != null) {
                    ...
                    final boolean isSeamlessAvailable = (mFastMode && device.isTrusted)
                                    ...
                                    getSelfExecutor().submit(new Runnable()
                                    {
                                        ...
                                        public void run()
                                        {
                                            ...
                                            TransferGroup.Assignee assignee = new TransferGroup.Assignee(
                                                    group, finalDevice, TransferObject.Type.INCOMING, connection);
                                            ...
                                            if (interrupter.interrupted())
                                                ...
                                                sendBroadcast(new Intent(ACTION_INCOMING_TRANSFER_READY)
                                                        .putExtra(EXTRA_GROUP_ID, groupId)
                                                        .putExtra(EXTRA_DEVICE_ID, finalDevice.id));

                                                if (isSeamlessAvailable)
                                                    try {
                                                        startTransferAsClient(group.id, finalDevice.id,
                                                                TransferObject.Type.INCOMING);
                                                    } catch (Exception e) {
                                                        e.printStackTrace();
                                                    }
                                                else
                                                    getNotificationHelper().notifyTransferRequest(finalDevice, group,
                            ...
                            case (Keyword.REQUEST_TRANSFER_JOB):
                                if (!device.isTrusted)
                                    ...
                                    try {
                                        ...
                                        Log.d(TAG, "Service handling transfer job with "
