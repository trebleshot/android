...
class ManualConnectionActivity : Activity(), ResultListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.confirm_button).setOnClickListener { v: View? ->
            ...
            val editable: Editable = editText.getText()
            if (editable == null || editable.length <= 0) editText.setError(getString(R.string.mesg_enterValidHostAddress)) else {
    ...
    private fun getEditText(): AppCompatEditText {
        return findViewById<AppCompatEditText>(R.id.editText)
    }
    ...
    class CheckAddressAsyncTask(private val listener: CheckHostnameListener) :
        android.os.AsyncTask<String?, Void?, InetAddress?>() {
        protected override fun doInBackground(vararg address: String): InetAddress? {
            try {
                if (address.size > 0) return InetAddress.getByName(address[0])
        ...
        override fun onPostExecute(address: InetAddress?) {
            ...
            if (address == null) listener.onHostnameError() else listener.onConnect(address)
        }
    }
