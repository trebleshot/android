...
class TransferAssigneeV12 : DatabaseObject<NetworkDeviceV12?> {
    ...
    override fun equals(obj: Any?): Boolean {
        if (obj is TransferAssigneeV12) {
            val otherAssignee = obj
    ...
    override fun getWhere(): SQLQuery.Select {
        return SQLQuery.Select(Kuick.Companion.TABLE_TRANSFERMEMBER)
            .setWhere(
                Kuick.Companion.FIELD_TRANSFERMEMBER_DEVICEID + "=? AND " +
                        Kuick.Companion.FIELD_TRANSFERMEMBER_TRANSFERID + "=?", deviceId, groupId.toString()
            )
    }

    override fun getValues(): ContentValues {
        val values = ContentValues()
        values.put(Kuick.Companion.FIELD_TRANSFERMEMBER_DEVICEID, deviceId)
        values.put(Kuick.Companion.FIELD_TRANSFERMEMBER_TRANSFERID, groupId)
        values.put(v12.Companion.FIELD_TRANSFERASSIGNEE_ISCLONE, 1)
        return values
    }
